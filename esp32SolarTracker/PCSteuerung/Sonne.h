...
// Kalenderdaten aus UTC + Zeitzonenversatz in Minuten (fuer die LCD-Anzeige).
Kalender kalenderZeit(uint32_t utcSekunden, int versatzMinuten);
// EU-Zeitzone aus UTC: +2 (MESZ) zwischen letztem So. Maerz und letztem So. Oktober, sonst +1.
int tagesversatz(uint32_t utcSekunden);
Sonnenwerte sonnenstand(uint32_t utcSekunden, int versatzMinuten, float breite, float laenge);
